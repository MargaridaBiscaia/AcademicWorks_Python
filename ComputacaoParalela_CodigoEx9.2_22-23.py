...
for g in range(nr_games):
  ...
  while pos:
    ...
    for i in range(1,len(Stat[pos])+1):
        if i==1:
            maximo_pos=i
            maximo=Stat[pos][maximo_pos]
        else:
            if(maximo<Stat[pos][i]):
                maximo_pos=i
                maximo=Stat[pos][maximo_pos]
            if(maximo==Stat[pos][i]):
                maximo_pos=random.choice([i, maximo_pos]) #utilização do random
                maximo=Stat[pos][maximo_pos]
...
Os resultados obtidos para 1000 peças foram
    ...
    No exercício 2, fizemos o teste 5 vezes para cada jogo
    - para 10 jogos: Erro na posição:  5 | 7 | 8 | 9 | 8 -> média 7,4
    - para 100 jogos: Erro na posição:  24 | 28 | 20 | 30 | 22 -> média 24,8
    - para 1000 jogos: Erro na posição:  106 | 95 | 72 | 88 | 93 -> média 90,8
    - para 10000 jogos: Erro na posição:  275 | 303 | 292 | 297 | 301-> média 293,6
    
Pelo que podemos concluir que a eficiência não muda significativamente.
"""
